...
class SAND():
    ...
    def __init__(self, pattern_length, subsequence_length, k=6):
        ...
        self.modelName = "SAND (online)"
        self.slidingWindow = pattern_length
    ...
    def fit(self, X, y=None, online=False, alpha=None, init_length=None, batch_size=None, overlaping_rate=10, verbose=False):
        ...
        if online:
            ...
            self.decision_scores_ = self._run(
                self.ts[:min(len(self.ts), self.current_time)])
            ...
            while self.current_time < len(self.ts)-self.subsequence_length:
                ...
                if self.current_time < len(self.ts)-self.subsequence_length:
                    ...
                    self.decision_scores_ += self._run(
                        self.ts[self.current_time-self.batch_size:])

            if verbose:
                print("[STOP]: score length {}".format(
                    len(self.decision_scores_)))
    ...
    def _run(self, ts):
        ...
        if len(self.nm_current_weight) != len(self.weights):
            self.nm_current_weight = self.nm_current_weight + \
                self.weights[len(self.nm_current_weight):]
        ...
        for sub_join, scores_sub_join, scores_sub_join_old, t_decay in zip(all_join, self.weights, self.nm_current_weight, self.time_decay):
            new_w = float(scores_sub_join) / \
                float(1+max(0, t_decay-self.batch_size))
            update_w = float(1-self.alpha)*float(scores_sub_join_old) + \
                float(self.alpha)*float(new_w)

            join = [float(j) + float(sub_j)*update_w for j,
                    sub_j in zip(list(join), list(sub_join))]
    ...
    def _run_next_batch(self):
        ...
        for i, (cur_c, t_a) in enumerate(zip(self.clusters, to_add)):
            ...
            if len(t_a) > 0:
                ...
                new_centroid, _ = self._extract_shape_stream(
                    all_sub_to_add, i, cur_c[0], initial=False)
                new_clusters.append(
                    (self._clean_cluster_tslearn(new_centroid), all_index))

                # Updating the intra cluster distance
                dist_to_add = self._compute_mean_dist(cur_c[0], all_index)
                ratio = float(len(cur_c[1])) / \
                    float(len(cur_c[1]) + len(all_index))
                all_mean_dist.append(
                    (ratio) * self.new_clusters_dist[i] + (1.0 - ratio) * dist_to_add)
    ...
    def _kshape_subsequence(self, initialization=True):
        ...
        for i in range(self.k):
            if len(cluster_subseq[i]) > 0:
                ...
                clusters.append((self._clean_cluster_tslearn(
                    ks.cluster_centers_[i]), cluster_idx[i]))
    ...
    def _set_normal_model(self):
        ...
        Frequency = list((np.array(Frequency) - min(Frequency)) /
                         (max(Frequency) - min(Frequency)+0.0000001)+1)
        Centrality = list((np.array(Centrality) - min(Centrality)) /
                          (max(Centrality) - min(Centrality)+0.0000001)+1)
