...
class metricor:
    ...
    def detect_model(self, model, label, contamination=0.1, window=100, is_A=False, is_threshold=True):
        ...
        if is_A is False:
            ...
            scoreX[math.ceil(window/2): len(score)+window -
                   math.floor(window/2)] = score
        else:
            scoreX = score
    ...
    def range_recall_new(self, labels, preds, alpha):
        ...
        for i in range_label:
            OverlapReward += self.w(i, p) * \
                self.Cardinality_factor(i, range_pred)
    ...
    def TPR_FPR_RangeAUC(self, labels, pred, P, L):
        ...
        # If there are no true segments (L is empty), return zeros for all metrics
        if len(L) == 0:
            return 0.0, 0.0, 0.0

        # P_new calculation
        # So TPR is neither too large nor too small
        ...
        # TPR_RangeAUC calculation
        TPR_RangeAUC = recall * existence_ratio

        # FP calculation
        FP = np.sum(pred) - TP

        # FPR_RangeAUC calculation
        N_new = len(labels) - P_new
        FPR_RangeAUC = FP / N_new

        # Precision_RangeAUC calculation
        if np.sum(pred) == 0:
            Precision_RangeAUC = 0.0
        else:
    ...
    def RangeAUC(self, labels, score, window=0, percentage=0, plot_ROC=False, AUC_type='window'):
        ...
        if AUC_type == 'window':
            ...
            labels = self.extend_postive_range_individual(
        ...
        TPR_list.append(1)
        # otherwise, range-AUC will stop earlier than (1,1)
        FPR_list.append(1)
