...
const AnomalyPlotter: React.FC<AnomalyPlotterInterface> = ({
  edgeName,
  groupName,
  index,
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(`key: ${edgeName}`);
        console.log(`anomalyEvent: `);
        console.log(anomalyEvent);

        console.log("anomalyDatapoints: ", anomalyDatapoints);
        setAnomalyEvent(anomalyEvent);
        if (anomalyEvent) {
          const dataPoints = anomalyEvent.anomalyDatapoints.map(
            (value, idx) => {
              const timestamp =
                new Date(anomalyEvent.createdAt).getTime() + idx * 60000; // Example to generate timestamps
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      type: "datetime",
      labels: {
        format: "dd MMM HH:mm", // Format the timestamp
      },
      axisBorder: {
        show: false,
      },
